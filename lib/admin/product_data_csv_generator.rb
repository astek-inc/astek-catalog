module Admin
  class ProductDataCsvGenerator < Admin::BaseProductDataCsvGenerator
        ...
        CSV.generate(headers: true) do |csv|
          ...
          design.variants_for_domain(website).each_with_index do |variant, i|
            ...
            variant.stock_items.rank(:row_order).for_domain(website).each do |stock_item|
              ...
              if @first_row
                csv << Admin::BaseProductDataCsvGenerator::PRIMARY_ROW_ATTRIBUTES.map{ |attr| (attr.nil? ? nil : attribute_value(attr, stock_item, full_variant_type, @image_index, website, nil, first_variant_row, substrate)) }
                @first_row = false
              else
                csv << Admin::BaseProductDataCsvGenerator::SECONDARY_ROW_ATTRIBUTES.map{ |attr| (attr.nil? ? nil : attribute_value(attr, stock_item, full_variant_type, @image_index, website, nil, first_variant_row, substrate)) }
              end
              ...
              if website == 'astekhome.com'
                ...
                unless design.collection.suppress_sample_option_from_display || (design.distributed? && variant.product_types.include?(murals))
                  csv << Admin::BaseProductDataCsvGenerator::SECONDARY_ROW_ATTRIBUTES.map { |attr| (attr.nil? ? nil : attribute_value(attr, stock_item, sample_variant_type, 0, website, nil, nil, substrate)) }
                end
              end
        ...
        if Admin::BaseProductDataCsvGenerator::TEXT_VALUES[attr.to_sym]
          ...
          when 'astekhome.com'
            if /sample/ =~ variant_type
              (BigDecimal(Admin::BaseProductDataCsvGenerator::SAMPLE_WEIGHT, 0) * BigDecimal('453.592', 0)).round.to_s
            else
              if custom_material
          ...
          when 'astekhome.com'
            if /sample/ =~ variant_type
              if variant.design.digital?
                Admin::BaseProductDataCsvGenerator::SAMPLE_PRICE_DIGITAL
              else
                Admin::BaseProductDataCsvGenerator::SAMPLE_PRICE_DISTRIBUTED
              end
            else
