class UpdateCollectionShimmer < ActiveRecord::Migration

  DATA_FILE = '2018-12-13_shimmer_update.csv'
  ...
  def up
    ...
    csv.each do |row|
      ...
      design.variants.each do |variant|
        variant.update_attributes!({backing_type: BackingType.find_by(name: item.backing)})
      end
